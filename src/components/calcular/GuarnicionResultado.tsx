...
import type { GuarnicionResult } from '../../lib/calculos'
import { useAppStore } from '../../store/useAppStore'

interface GuarnicionResultadoProps { nombre: string; totalPacientes: number; resultado: GuarnicionResult }
function fmtG(value: number): string { return value >= 1000 ? `${(value / 1000).toFixed(1)} kg` : `${value} g` }

export default function GuarnicionResultado({ nombre, totalPacientes, resultado }: GuarnicionResultadoProps) {
  const [guardado, setGuardado] = useState(false)
  const user = useAppStore((state) => state.user)
  const servicio = useAppStore((state) => state.servicio)
  const { addRegistro } = useHistorial(user?.id)
  const guardar = async (): Promise<void> => {
    const response = await addRegistro({ plato: nombre, servicio: servicio === 'almuerzo' ? 'Almuerzo' : 'Cena', raciones: totalPacientes, categoria: 'guarnicion' })
