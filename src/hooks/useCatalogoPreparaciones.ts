...
export function useCatalogoPreparaciones(): UseCatalogoReturn {
  ...
  useEffect(() => { listeners.add(setData); if (!cache) void reload(); return () => { listeners.delete(setData) } }, [reload])
  ...
  const save = useCallback(async (input: PreparacionCatalogoInput, id?: string) => {
    ...
    if (!token) return { error: 'Inicia sesión para guardar el catálogo' }
    try {
    ...
    } catch (cause) { const detail = message(cause); setError(detail); return { error: detail } }
  }, [token])

  const archive = useCallback(async (id: string) => {
    if (!token) return { error: 'Inicia sesión para archivar el catálogo' }
